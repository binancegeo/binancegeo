...
uint256 hashGenesisBlock("0xe82e85fd1610e6c84a633e31c38b6ce5a6ed3a46241bceae6ed0db42b66d707c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Titaniumcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Titaniumcoin Signed Message:\n";
...
{
    // Titaniumcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 300000); // Titaniumcoin: 300k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Titaniumcoin: 2 days
static const int64 nTargetSpacing = 2 * 60; // Titaniumcoin: 2 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Titaniumcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc66f54ef791688cc8db2451bab2c9284f6b77554978222819d94a151333cb4c1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12/12/2023";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a1137628e576ab88d43eb32b547f1f6f6933bfa28813ef72269e02b8659ba7698adfcdb68eda265419422a1c3446187ab6e537228b110b1969dbbed25be35ddd") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1702403201;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088593927 ;

        if (fTestNet)
        {
            block.nTime    = 1702403179;
            block.nNonce   = 387157207;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xfe2820b8d90e0cf04b893f4b9cd2a811ee9e9170d73983214623094f7f1a4ff9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb1, 0xdd }; // Titaniumcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TitaniumcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TitaniumcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TitaniumcoinMiner(CWallet *pwallet)
{
    printf("TitaniumcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("titaniumcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TitaniumcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TitaniumcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TitaniumcoinMiner, pwallet));
}
